...
const getCategoryStyles = (category: string, isActive: boolean) => {
  ...
  const styles: Record<string, { bg: string; text: string; border: string; activeBg: string; activeText: string; activeBorder: string }> = {
    'phone accessories': {
      bg: 'bg-sky-50', text: 'text-sky-700', border: 'border-sky-200',
      activeBg: 'bg-sky-600', activeText: 'text-white', activeBorder: 'border-sky-600'
    },
    'computer parts': {
      bg: 'bg-indigo-50', text: 'text-indigo-705', border: 'border-indigo-200',
      activeBg: 'bg-indigo-600', activeText: 'text-white', activeBorder: 'border-indigo-600'
    },
    'electric items': {
      bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-250',
      activeBg: 'bg-amber-600', activeText: 'text-white', activeBorder: 'border-amber-600'
    },
    'drink items': {
      bg: 'bg-cyan-50', text: 'text-cyan-700', border: 'border-cyan-200',
      activeBg: 'bg-cyan-600', activeText: 'text-white', activeBorder: 'border-cyan-600'
    },
    'food items': {
      bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-250',
      activeBg: 'bg-emerald-600', activeText: 'text-white', activeBorder: 'border-emerald-600'
    },
    'grocery items': {
      bg: 'bg-orange-50', text: 'text-orange-700', border: 'border-orange-200',
      activeBg: 'bg-orange-600', activeText: 'text-white', activeBorder: 'border-orange-600'
    },
    'phone cards & reload': {
      bg: 'bg-rose-50', text: 'text-rose-700', border: 'border-rose-200',
      activeBg: 'bg-rose-600', activeText: 'text-white', activeBorder: 'border-rose-600'
    },
    'gift items': {
      bg: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-250',
    ...
    'all': {
      bg: 'bg-slate-50', text: 'text-slate-655', border: 'border-slate-200',
      activeBg: 'bg-slate-800', activeText: 'text-white', activeBorder: 'border-slate-800'
    }
  };

  const match = styles[normalized] || {
    bg: 'bg-slate-50', text: 'text-slate-655', border: 'border-slate-200',
    activeBg: 'bg-slate-850', activeText: 'text-white', activeBorder: 'border-slate-850'
  };

  return isActive 
    ? `${match.activeBg} ${match.activeText} ${match.activeBorder}`
    : `${match.bg} ${match.text} ${match.border} hover:bg-slate-100/70`;
};

const getCategoryBorderColor = (category: string) => {
  const normalized = category.toLowerCase().trim();
  const colorMap: Record<string, string> = {
    'phone accessories': 'border-t-sky-500',
    'computer parts': 'border-t-indigo-500',
    'electric items': 'border-t-amber-500',
    'drink items': 'border-t-cyan-500',
    'food items': 'border-t-emerald-500',
    'grocery items': 'border-t-orange-500',
    'phone cards & reload': 'border-t-rose-500',
    'gift items': 'border-t-purple-550',
    'services': 'border-t-blue-500'
  };
  return colorMap[normalized] || 'border-t-slate-400';
};
    ...
    return (
      <div className="space-y-3 flex flex-col justify-between h-full">
        <div className="space-y-3 flex-1 flex flex-col min-h-0">
          {/* Header */}
          <div className="flex flex-col gap-2 pb-2.5 border-b border-slate-100 shrink-0">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center space-x-1.5">
                <button
                  type="button"
                  id={isMobile ? "pos-btn-clear-cart-mobile" : "pos-btn-clear-cart"}
                  onClick={() => {
          ...
          {/* Cart Item Rows */}
          <div className={isMobile ? "space-y-1.5 flex-1 overflow-y-auto pr-1 max-h-[30vh]" : "space-y-1.5 min-h-[220px] max-h-[280px] overflow-y-auto pr-1"}>
            {posCart.length === 0 ? (
        ...
        <div className="border-t border-slate-200 pt-3 space-y-3 shrink-0">
              ...
              {selectedCustomer && selectedCustomer.loyaltyPoints > 0 && (
                <div className="flex justify-between items-center py-1 border-t border-dashed border-slate-100 mt-1">
                  <div className="flex items-center space-x-1.5">
                    <input
                      type="checkbox"
                      id={isMobile ? "redeemPointsCheckbox-mobile" : "redeemPointsCheckbox"}
                      checked={redeemPoints}
                      onChange={(e) => setRedeemPoints(e.target.checked)}
                      className="h-3.5 w-3.5 text-indigo-600 border-slate-350 rounded focus:ring-indigo-500 cursor-pointer"
                    />
                    <label htmlFor={isMobile ? "redeemPointsCheckbox-mobile" : "redeemPointsCheckbox"} className="text-[10px] font-extrabold text-indigo-650 cursor-pointer hover:text-indigo-800">
                      {language === 'en' 
            ...
            {paymentMethod === 'Cheque' ? (
              <div className="px-3 py-2.5 space-y-2 bg-slate-50/50">
                ...
                {chequeAmount > totals.total && (
                  <div className="p-2 bg-emerald-50 border border-emerald-200 rounded-lg text-[9px] font-black text-emerald-800 flex justify-between items-center">
                    <span>{language === 'en' ? 'Return Change in Cash:' : 'පාරිභෝගිකයාට දිය යුතු ඉතිරිය (සසල්ලි වලින්):'}</span>
              ...
              <div className="px-3 py-2.5 space-y-1.5 bg-slate-50/50">
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    type="text"
                    id={isMobile ? "payment-reference-input-mobile" : "payment-reference-input"}
          ...
          {/* Complete Sale CTA */}
          <button
            id={isMobile ? "pos-btn-complete-sale-mobile" : "pos-btn-complete-sale"}
            onClick={handleCompleteSale}
            disabled={posCart.length === 0 || (paymentMethod === 'Pending' && !selectedCustomer)}
            className={`w-full py-4 rounded-2xl text-xs uppercase tracking-wider font-black shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${
          ...
          >
            ...
            <span>
              {paymentMethod === 'Pending' && !selectedCustomer
                ...
                : (language === 'en' 
                    ? `CHARGE Rs. ${totals.total.toLocaleString(undefined, { maximumFractionDigits: 0 })}` 
                    : `අයකිරීම රු. ${totals.total.toLocaleString(undefined, { maximumFractionDigits: 0 })}`
                  )}
            </span>
          </button>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4 block">
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 space-y-3 sticky top-[0px] z-20">
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1.5 scrollbar-none snap-x snap-mandatory border-t border-slate-100 pt-3">
              <button
                type="button"
                onClick={() => setSelectedCategory('All')}
                className={`snap-start px-3 py-1.5 rounded-xl border text-[10px] font-extrabold transition-all shrink-0 uppercase tracking-wider ${getCategoryStyles('All', selectedCategory === 'All')}`}
              >
                {t.allCategories}
              </button>
              {categories.map(cat => {
                ...
                return (
                  <button
                    ...
                    className={`snap-start px-3 py-1.5 rounded-xl border text-[10px] font-extrabold transition-all shrink-0 uppercase tracking-wider ${getCategoryStyles(cat, isActive)}`}
                  >
                    {(translations[language] as any)[cat] || cat}
                  </button>
                );
              })}
            </div>
            ...
            {paginatedProducts.map(p => {
              ...
              return (
                <button
                  ...
                  className={`relative p-3 bg-white border-t-4 border-r border-b border-l rounded-xl text-left hover:shadow-md active:scale-95 transition flex flex-col justify-between h-36 ${
                    isOutOfStock 
                      ? 'border-t-slate-205 border-r-slate-100 border-b-slate-100 border-l-slate-100 bg-slate-50/50 opacity-50 cursor-not-allowed' 
                      : cartQty > 0
                      ? `${getCategoryBorderColor(p.category)} border-r-emerald-500 border-b-emerald-500 border-l-emerald-500 ring-1 ring-emerald-500`
                      : isLowStock
                      ? `${getCategoryBorderColor(p.category)} border-r-rose-250 border-b-rose-250 border-l-rose-250`
                      : `${getCategoryBorderColor(p.category)} border-r-slate-200 border-b-slate-200 border-l-slate-200 hover:border-r-blue-400 hover:border-b-blue-400 hover:border-l-blue-400`
                  }`}
                >
                  {cartQty > 0 && (
                    <div className="absolute -top-2 -right-2 bg-emerald-600 text-white text-[10px] font-black rounded-full h-5.5 w-5.5 flex items-center justify-center border-2 border-white shadow-md z-10 animate-bounce">
                      {p.isWeighted ? `${cartQty}kg` : cartQty}
                    </div>
                  )}
                  <div>
      ...
      <div className="hidden lg:flex lg:col-span-5 bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex-col justify-between">
        {renderCartContent(false)}
      </div>

      {/* Mobile Floating Bottom Summary Bar */}
      {posCart.length > 0 && (
        <div className="lg:hidden fixed bottom-4 left-4 right-4 z-30 shadow-lg">
          <button
            type="button"
            onClick={() => setIsMobileCartOpen(true)}
            className="w-full bg-slate-900 hover:bg-slate-950 text-white font-extrabold px-4 py-3.5 rounded-2xl flex items-center justify-between transition-all active:scale-[0.98]"
          >
            <div className="flex items-center space-x-2">
              <div className="relative">
                <ShoppingCart className="h-5 w-5 text-emerald-400" />
                <span className="absolute -top-2 -right-2 bg-emerald-500 text-white text-[9px] font-black rounded-full h-4 w-4 flex items-center justify-center">
                  {posCart.length}
                </span>
              </div>
              <span className="text-xs">{language === 'en' ? 'View active bill' : 'ක්‍රියාකාරී බිල පෙන්වන්න'}</span>
            </div>
            <div className="flex items-center space-x-1.5">
              <span className="text-xs font-black text-emerald-400">Rs. {totals.total.toLocaleString()}</span>
        ...
        {isMobileCartOpen && (
          <>
            ...
            <motion.div
              ...
              className="lg:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-35"
            />
            ...
            <motion.div
              ...
              className="lg:hidden fixed inset-x-0 bottom-0 z-40 bg-white rounded-t-3xl shadow-2xl border-t border-slate-200 h-[85vh] max-h-[85vh] flex flex-col justify-between overflow-hidden"
            >
              {/* Drag handle header */}
              <div className="flex flex-col items-center py-2.5 border-b border-slate-100 bg-slate-50 cursor-pointer shrink-0" onClick={() => setIsMobileCartOpen(false)}>
                <div className="w-12 h-1.5 bg-slate-300 rounded-full mb-2"></div>
                <div className="flex justify-between items-center w-full px-4">
                  <h3 className="text-xs font-bold text-slate-800 flex items-center">
                    <ShoppingCart className="h-4 w-4 mr-1 text-blue-600" />
                    {language === 'en' ? `Active Bill (${posCart.length} items)` : `කරත්තය (${posCart.length} බඩු)`}
                  </h3>
                  <button type="button" onClick={() => setIsMobileCartOpen(false)} className="text-slate-400 hover:text-slate-800 text-xs font-extrabold px-2 py-0.5 rounded-lg hover:bg-slate-200 transition">
                    ✕
                  </button>
                </div>
              </div>
