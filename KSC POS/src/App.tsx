...
function App() {
  ...
  const [viewMode, setViewMode] = useState<'storefront' | 'admin' | 'super-admin'>('admin');
  ...
  // Subscription Active Check on startup (Disabled for KSC POS Offline edition)
  useEffect(() => {
  ...
  return (
    <div className="h-screen lg:min-h-screen lg:h-auto bg-slate-50 flex flex-col overflow-hidden lg:overflow-auto text-slate-800">
      {/* Navigation Header */}
      <Navbar
        language={language}
      ...
      {/* Main Content Body */}
      <main className="flex-1 max-w-[100vw] w-full overflow-y-auto lg:overflow-visible px-0 lg:px-8 py-0 lg:py-6 pb-24 lg:pb-6">
        {viewMode === 'storefront' ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 lg:gap-6">
            ...
            {/* Admin Content Area */}
            <div className={`col-span-1 ${isPosFullScreen ? 'lg:col-span-11' : 'lg:col-span-9'} transition-all duration-300 px-3 py-3 lg:px-0 lg:py-0`}>
      ...
      {/* Footer — hidden on mobile (app mode), visible on desktop only */}
      <footer className="hidden lg:block bg-slate-950 text-slate-400 text-xs py-8 border-t border-slate-900 mt-12 w-full print:hidden">
      ...
      {/* Mobile Floating Bottom Navigation Bar (Visible only on lg:hidden) */}
      {viewMode === 'admin' && (
        <div className="lg:hidden fixed bottom-4 left-4 right-4 z-40 bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-2xl shadow-xl p-1 flex justify-around items-center">
          <button
            onClick={() => {
              setAdminTab('pos');
              setAdminSubTab('');
            ...
            className={`flex flex-col items-center justify-center py-2 px-3 rounded-xl transition ${
              adminTab === 'pos' && !isMobileMoreOpen ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <ShoppingCart className="h-5 w-5" />
            <span className="text-[9px] font-bold mt-1">POS</span>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-2 px-3 rounded-xl transition ${
              adminTab === 'quotations' && adminSubTab === 'repairs' && !isMobileMoreOpen ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Wrench className="h-5 w-5" />
            <span className="text-[9px] font-bold mt-1">{language === 'en' ? 'Repairs' : 'රෙපෙයාර්'}</span>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-2 px-3 rounded-xl transition ${
              adminTab === 'inventory' && !isMobileMoreOpen ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Laptop className="h-5 w-5" />
            <span className="text-[9px] font-bold mt-1">{language === 'en' ? 'Inventory' : 'තොග'}</span>
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center py-2 px-3 rounded-xl transition ${
              adminTab === 'reports' && !isMobileMoreOpen ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <BarChart3 className="h-5 w-5" />
            <span className="text-[9px] font-bold mt-1">{language === 'en' ? 'Reports' : 'වාර්තා'}</span>
          </button>

          <button
            onClick={() => setIsMobileMoreOpen(prev => !prev)}
            className={`flex flex-col items-center justify-center py-2 px-3 rounded-xl transition ${
              isMobileMoreOpen ? 'bg-indigo-650 text-white shadow' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            <Menu className="h-5 w-5" />
            <span className="text-[9px] font-bold mt-1">{language === 'en' ? 'More' : 'තවත්'}</span>
          </button>
        </div>
      )}

      {/* Mobile "More" Fullscreen Overlay Grid */}
      {isMobileMoreOpen && viewMode === 'admin' && (
        <div className="lg:hidden fixed inset-0 z-30 bg-slate-950/95 backdrop-blur-md pt-20 pb-28 px-6 overflow-y-auto flex flex-col justify-between animate-in fade-in duration-200">
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div>
                <h2 className="text-base font-extrabold text-white">KSC POS Modules</h2>
                <p className="text-[10px] text-slate-400 font-medium">Select a system module to continue.</p>
              </div>
              <button 
            ...
            <div className="grid grid-cols-2 gap-3.5 pt-2">
              {[
                { tab: 'dashboard', sub: '', label: language === 'en' ? 'Dashboard' : 'ප්‍රධාන පුවරුව', desc: 'Analytics & summaries', icon: TrendingUp, color: 'from-blue-600/20 to-blue-500/10 border-blue-500/20 text-blue-400' },
                { tab: 'contacts', sub: 'customers', label: language === 'en' ? 'Contacts & Loyalty' : 'පාරිභෝගිකයින්', desc: 'Manage customers & VIPs', icon: Users, color: 'from-emerald-600/20 to-emerald-500/10 border-emerald-500/20 text-emerald-400' },
                { tab: 'purchases', sub: 'purchases', label: language === 'en' ? 'Purchases & Stock' : 'මිලදී ගැනීම් & තොග', desc: 'Supplier orders & stock adjust', icon: Truck, color: 'from-amber-600/20 to-amber-500/10 border-amber-500/20 text-amber-400' },
                { tab: 'special-orders', sub: '', label: language === 'en' ? 'Special Orders' : 'විශේෂ ඇණවුම්', desc: 'Track custom orders', icon: ShoppingBag, color: 'from-rose-600/20 to-rose-500/10 border-rose-500/20 text-rose-400' },
                { tab: 'attendance', sub: 'profiles', label: language === 'en' ? 'Staff & HR' : 'සේවකයින් & HR', desc: 'Attendance, profiles, salary', icon: UserCheck, color: 'from-violet-600/20 to-violet-500/10 border-violet-500/20 text-violet-400' },
                { tab: 'settings', sub: 'shop', label: language === 'en' ? 'System Settings' : 'සිටින්ස් (Settings)', desc: 'Shop details & hardware', icon: Settings, color: 'from-slate-600/20 to-slate-500/10 border-slate-500/20 text-slate-300' },
                { tab: 'insights', sub: '', label: language === 'en' ? 'Architect Insights' : 'නිර්මාණ සැලසුම්', desc: 'View design systems', icon: Layers, color: 'from-indigo-600/20 to-indigo-500/10 border-indigo-500/20 text-indigo-400' },
                { tab: 'backup', sub: '', label: language === 'en' ? 'Database Backup' : 'පද්ධති උපස්ථය', desc: 'Local backups & rollbacks', icon: Download, color: 'from-teal-600/20 to-teal-500/10 border-teal-500/20 text-teal-400' }
              ].map(item => {
                const Icon = item.icon;
                ...
                return (
                  <button
                    key={item.tab}
                    onClick={() => {
                      setAdminTab(item.tab as any);
                      setAdminSubTab(item.sub);
                      setIsMobileMoreOpen(false);
                    }}
                    className={`flex flex-col items-start p-3.5 rounded-2xl border bg-gradient-to-br text-left transition duration-200 active:scale-[0.97] ${item.color} ${
                      isActive ? 'ring-2 ring-indigo-500 ring-offset-2 ring-offset-slate-950' : ''
                    }`}
                  >
                    <Icon className="h-5 w-5 mb-2 shrink-0" />
                    <div className="font-extrabold text-[11px] text-white leading-tight">{item.label}</div>
                    <div className="text-[8.5px] text-slate-400 mt-0.5 leading-tight">{item.desc}</div>
                  </button>
                );
              })}
            </div>
